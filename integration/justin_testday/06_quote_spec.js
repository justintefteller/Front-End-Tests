...
describe('Quote Tab', function () {
    ...
    it('Fills Out All Quote Info', function () {
        ...
        cy.get('#quote-lines > tbody > tr').first().then($line => {
            ...
            cy.get('#edit-line-form').within(() => {
                cy.get('.qty_break_row').last().then($row => {
                    let row = Cypress.$($row).attr('row')
                    cy.get('#qty_' + row).clear().type(100)
                    cy.get('#resale_' + row).last().then($resale => {
                        cy.wrap($resale).wait(2000).clear({force:true}).type(' ', {force:true})
                        cy.wrap($resale).wait(2000).clear({force:true}).type(100,{force:true})
                    })
    ...
    it("Checks the quote pdf", function(){
        var header = [ 
            ...
            ["Order Type", "Unscheduled", 'FOB', "S - Shipment"],
        ...
        cy.get('#pdf_preview').within(() => {
            cy.get('tbody > tr').each((tr, idx) => {
                // loop through all the rows of the header & compare the above header array to see
                if(idx != 4 && idx < 7){
                    var index;
                    idx >= 4 ? index = idx -1 : index = idx; //  index 4 is blank. 
                                                             // it's between Carrier Account & Comments. 
                                                             // could have thrown a blank array in there, but that'd also be confuse
                    cy.wrap(header[index]).each(itm => {   // loops through 'header' & reads the keys to check the value from above
                        // each row should include item
        ...
        //footer check
        var line = ["CMP1", "CMP","1", 100,"$100.00","$0.00","$0.00" ,"$10,000.00"]
    ...
    it('Deletes line, Creates Non-Existing Part on Quote, Clones line', function () {
    ...
    //can't get the iFrame to work still
    //might be smarter to use the api and load a customer then test that way
    ...
    it('Clones the Quote and Changes All Line Feature', function () {
        ...
        cy.server();
        cy.route('POST', '/quote/*/ajax_update_all_lines').as('all_lines')
