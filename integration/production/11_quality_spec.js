...
describe("The Inventory Tab", function () {
  ...
  it("Logs An Inspection (Pass)", function () {
    ...
    cy.get('#inspector_user_id').then(elem => {
        elem.val('1');
    });
  ...
  it("Edits CAR", function () {
    ...
    nav.get_dropdown("Special Corrective Reason")
        ...
        cy.get($this).within($that => {
          cy.get("tr").each(($tr, $index, $arr) => {
            if ($index == 0) {
              ...
              cy.get('input[name="identified_on"]').should("have.value", today2);
  ...
  const input_names = 
    ...
    const print_assert = [
        ...
        ["NCR Type:","Customer Complaint",	"Initiated By:","techx", today2],
