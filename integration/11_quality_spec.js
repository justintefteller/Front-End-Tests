...
describe("The Inventory Tab", function () {
      ...
      .within($this => {
        ...
        cy.get("td").each(($td, $idx) => {
          if ($idx == 0) {
            ...
            cy.get(`${classes[$idx]}`).should("contain", `${content[$idx]}`);
          }
        });
      });
  });
      ...
      if ($idx == 0) {
        cy.get($this).within($that => {
          cy.get("tr").each(($tr, $index, $arr) => {
            if ($index == 0) {
              cy.get('select[name="type_id"]').invoke("val").should("eq", "1");
              cy
        ...
        cy.get($this).within($that => {
          cy.get("tr").each(($tr, $index) => {
            if ($index == 1) {
              cy.get("#resolution_type_id").invoke("val").should("eq", "1");
            } else if ($index == 2) {
              cy
  ...
  it("Creates a NCR and CAR from scratch", function () {
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      cy
        .get(".status_message")
        .should("contain", `${links[i].toUpperCase()} Created`);
    }
  });
  ...
  it("Failed Inspections", function () {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      cy.get(".last_entry_success").should("contain", "NCR");
      cy.get(".last_entry_success").find("a").first().click();
      cy
  ...
  it("Preforms a Vendor Return", function () {
    ...
    cy.get("input[name*='return_qty_']").invoke("val").should("eq", "10");
