...
describe("The Order Entry Stock Tab", function () {
    ...
    it('Edits the Header', function () {
        ...
        cy.get_quote_po_or_order_num('order_num', '', 1);
        nav.side_menu('Edit')
    ...
    it('Deletes The Line', function() {
        nav.side_menu('Delete Line')
        cy.get('#order-lines > tbody > tr').each(($tr) => {
            if($tr.text().includes('CMP2')){
                cy.get($tr).within(() => {
                    cy.get('input').check();
                })
            }
    ...
    it('Picks Everything and Invoices', function() {
        ...
        cy.window().then($win => {
            var pick_parts_url = $win.location.pathname;
            $win.sessionStorage.setItem('pick_parts_url', pick_parts_url);
        })
        ...
        cy.window().then($win => {
            cy.visit($win.sessionStorage.getItem('pick_parts_url'))
        })
    ...
    it('Create Outsource PO', function() {
        ...
        var po_line = [
            "1", "Stuff And More", "PRTABC1234",today,"5", "(qty is locked to",	"$10.00","$50.00", "Open", "sup"
        ]
        nav.side_menu('Order')
        nav.side_menu('Workorder')
